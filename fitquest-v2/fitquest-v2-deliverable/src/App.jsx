...
import { auth, signInWithGoogle, signOutUser, isFirebaseConfigured, onAuthStateChanged } from "./firebase";
...
// ─── LOGIN SCREEN ─────────────────────────────────────────────────────────────
function LoginScreen({ onGoogleSignIn, onSkip }) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 20px" }}>
      <div style={{ maxWidth: 360, width: "100%", textAlign: "center" }} className="slide-up">
        <div style={{ fontSize: 64, marginBottom: 16 }}>🏋️</div>
        <h1 style={{ fontSize: 34, fontWeight: 900, marginBottom: 8 }}>FitQuest</h1>
        <p style={{ color: T.muted, marginBottom: 48, fontSize: 15 }}>Your personal fitness companion</p>
        ...
        <button
          ...
          style={{
            ...
            marginBottom: 12, cursor: loading ? "wait" : "pointer",
            opacity: loading ? 0.7 : 1, border: "none", boxShadow: "0 1px 4px rgba(0,0,0,0.3)",
          }}
        >
          <GoogleIcon />
        ...
        <button
          ...
          style={{
            ...
            color: T.muted, fontWeight: 500, fontSize: 15, cursor: "pointer",
        ...
        {error && <p style={{ color: T.danger, marginTop: 16, fontSize: 14 }}>{error}</p>}
      </div>
    </div>
  );
}
...
// ─── ONBOARDING ───────────────────────────────────────────────────────────────
function Onboarding({ onComplete, prefillName = "" }) {
  const [step, setStep] = useState(0);
  const [form, setForm] = useState({ name: prefillName, gender: "All", goal: GOALS[0], experience: EXPERIENCE_LEVELS[0] });
      ...
      <Card style={{ textAlign: "center", marginBottom: 16 }}>
        {authUser?.photoURL ? (
          <img
            ...
            alt={user?.name}
            style={{ width: 72, height: 72, borderRadius: "50%", margin: "0 auto 14px", display: "block", objectFit: "cover" }}
          />
        ) : (
          <div style={{
        ...
        {authUser && <div style={{ color: T.muted, fontSize: 13, marginBottom: 4 }}>{authUser.email}</div>}
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (!u && isFirebaseConfigured) setShowLogin(true);
  ...
  async function handleGoogleSignIn() {
    ...
    setAuthUser(firebaseUser);
    setShowLogin(false);
    // If they already have a saved profile, go straight to app
    const existing = UserService.get();
    if (existing) setUser(existing);
  }

  function handleOnboarding(profile) {
    const finalProfile = authUser
      ? { ...profile, photoURL: authUser.photoURL, email: authUser.email }
      : profile;
    UserService.set(finalProfile);
  ...
  async function handleSignOut() {
    ...
    setAuthUser(null);
    handleReset();
    if (isFirebaseConfigured) setShowLogin(true);
  }
  ...
  if (showLogin) {
    return (
      <>
        <GlobalStyle />
        <LoginScreen onGoogleSignIn={handleGoogleSignIn} onSkip={() => setShowLogin(false)} />
      </>
    );
  }
    ...
    return (
      <>
        ...
        <Onboarding
          ...
          prefillName={authUser?.displayName ?? ""}
        />
      </>
    );
  }
